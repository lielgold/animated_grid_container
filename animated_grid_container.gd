...
# Grid variables:
@export var rows = 1: get = get_rows, set = set_rows # (int, 1, 1024, 1)
@export_enum("Horizontal", "Vertical") var fill_priority :int :set = set_fill_priority
@export_enum("Begin", "End") var horizontal_fill_direction = 1: set = set_horizontal_fill_direction
@export_enum("Begin", "End") var vertical_fill_direction = 1: set = set_vertical_fill_direction
@export var horizontal_separation: float: set = set_horizontal_separation
@export var vertical_separation: float: set = set_vertical_separation
# Animation variables:
@export_range(0, 255) var update_delay:float # (float, 0, 255)
@export_range(0, 255) var update_time:float # (float, 0, 255)
@export var update_ease :float= 1 # (float, EASE)
...
func get_grid_position(id, _max_rows):
	var result = Vector2();
	if fill_priority:
...
func make_empty_main_rows_expanded(visible_controls, max_rows, row_expanded_x:PackedInt32Array, row_expanded_y:PackedInt32Array):
	if fill_priority:
...
func evaluate_row_remaining_space(size_a, max_min:Dictionary, row_expanded:Array, max_rows:int, separation:float):
	for k in max_min.keys():
		if row_expanded.has(k) == false:
			size_a -= max_min[k]
	size_a -= separation * max(max_rows - 1, 0)
	...
	while (can_fit == false and row_expanded.size() > 0):
		...
		for c in row_expanded:
			if max_min[c] > max_min[max_value_index]:
				max_value_index = c
			if can_fit and (size_a / row_expanded.size()) < max_min[c]:
				can_fit = false
		if can_fit == false:
			row_expanded.erase(max_value_index)
			size_a -= max_min[max_value_index]
	return size_a
